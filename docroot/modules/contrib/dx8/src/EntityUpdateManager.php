 ...
 *
 * Whenever a DX8 entity is saved, this service checks the entity to see if any of the EntityUpdate plugins need to be
 * run. If they are, the entity JSON is updated and the name of the latest EntityUpdate plugin is added to the entity
 * so they are only run once.
 *
 * New update callbacks shoud be defined as new EntityUpdate plugins (see /src/Plugin/EntityUpdate/*)
 *
...
class EntityUpdateManager {
   ...
   * Entrypoint.
   *
   * This should check 'last_entity_update' and if it's blank (a new entity) set the latest available update so nothing
   * gets run for this entity.
   *
  ...
  public function apply(EntityUpdateInterface &$entity) {
    ...
    if (!$is_new || (!is_null($entity->getLastAppliedUpdate()) && isset($all_plugins[$entity->getLastAppliedUpdate()]))) {
      if (isset($all_plugins[$entity->getLastAppliedUpdate()])) {
        ...
        $start_index = 0; // Couldn't find the class method defined in the entity.
      }
      ...
      foreach ($all_plugins as $plugin_id => $definition) {
        ...
        if ($start_index < $plugin_index) {
          ...
          if ($instance = $this->getPluginInstance($plugin_id)) {
            ...
            $applied[] = $plugin_id; // For unit tests.
          }
        }
      }
    }
    ...
    return $applied;  // See EntityupdateManagerUnitTest.
  }
